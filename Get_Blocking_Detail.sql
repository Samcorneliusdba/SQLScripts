select 
	...
	,CASE WHEN (Req2.session_id IS NOT NULL AND (sess.session_id IS NULL OR Req.blocking_session_id=0 or req.blocking_session_id is null)) THEN 1 ELSE 0 END [IsHeadBlocker]
	...
	ELSE 'UNKNOWN' END [transaction_isolation_level]
...
Inner Join sys.dm_os_tasks tasks on (req.session_id=tasks.session_id)
