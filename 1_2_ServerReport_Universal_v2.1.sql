...
INSERT INTO dbo.##ReviewResults SELECT 'SERVER CONFIGURATION INFORMATION', '','','','','','','','','','','','','','','','', '','','','','','','','','','','','';
INSERT INTO dbo.##ReviewResults SELECT '', '','','','','','','','','','','','','','','','', '','','','','','','','','','','','';
...
DECLARE @Instversion VARCHAR(255)
...
DECLARE @MajorVersion varchar(255)
DECLARE @ProductLevel varchar(255)
DECLARE @IntVersion varchar(25)
...
SET @MajorVersion = ( SELECT
  CASE 
     WHEN CONVERT(VARCHAR(128), SERVERPROPERTY ('productversion')) like '8%' THEN 'Micorosoft SQL Server 2000 '
     WHEN CONVERT(VARCHAR(128), SERVERPROPERTY ('productversion')) like '9%' THEN 'Micorosoft SQL Server 2005 '
     WHEN CONVERT(VARCHAR(128), SERVERPROPERTY ('productversion')) like '10.0%' THEN 'Micorosoft SQL Server 2008 '
     WHEN CONVERT(VARCHAR(128), SERVERPROPERTY ('productversion')) like '10.5%' THEN 'Micorosoft SQL Server 2008 R2 '
     WHEN CONVERT(VARCHAR(128), SERVERPROPERTY ('productversion')) like '11%' THEN 'Micorosoft SQL Server 2012 '
     WHEN CONVERT(VARCHAR(128), SERVERPROPERTY ('productversion')) like '12%' THEN 'Micorosoft SQL Server 2014 '
     WHEN CONVERT(VARCHAR(128), SERVERPROPERTY ('productversion')) like '13%' THEN 'Micorosoft SQL Server 2016 '     
     ELSE 'unknown'
  END AS MajorVersion)

SET @IntVersion = (select substring(cast(serverproperty('productversion') as varchar(25)),0,charindex('.',cast(serverproperty('productversion') as varchar(25)),0)))
...
--print @MajorVersion
 ...
 set @Instversion = ( SELECT @MajorVersion + '' + (SELECT @ProductLevel) )
...
Insert into ##ServerReview  values (@sname, @insname, @Instversion, @Edition, @Platform, @Processors,@WindowsVersion , @OSVersion, @starttime, @IsClustered, @ActiveClusterNode, @IsFullTextInstalled, @IsIntegratedSecurityOnly)


INSERT INTO dbo.##ReviewResults SELECT 'ServerName' , 'Instance Name' , 'Version Name', 'Edition', 'Platform' , 'Processors' , 'WindowsVersion' , 'OSVersion', 'StartTime' ,  'IsClustered', 'Active Cluster Node' , 'IsFullTextInstalled' , 'IsIntegratedSecurityOnly','','','','','','','','','','','','','','','','';							
...
INSERT INTO dbo.##ReviewResults SELECT '', '','','','','','','','','','','','','','','','', '','','','','','','','','','','','';;
INSERT INTO dbo.##ReviewResults SELECT 'VERSION INFORMATION', '','','','','','','','','','','','','','','','', '','','','','','','','','','','','';
INSERT INTO dbo.##ReviewResults SELECT '', '','','','','','','','','','','','','','','','', '','','','','','','','','','','','';;


SET @versionnumber = CONVERT(varchar,SERVERPROPERTY('productversion'));

IF @versionnumber = '13.0.4001.0' OR @versionnumber = '12.0.5000.0' OR @versionnumber = '11.00.6020.0' OR @versionnumber = '10.50.6000.34' OR @versionnumber = '10.0.6000.29' OR @versionnumber = '9.0.5000.0'
	SET @notupgradable = 1
ELSE
...
IF @versionnumber LIKE '%13.0.4%' SET @notupgradable = 1
...
ELSE
	INSERT INTO dbo.##ReviewResults SELECT @versionnumber, CASE @notupgradable WHEN 1 THEN 'Latest version' ELSE 'Updates available' END,CASE @version when 16 then '13.0.4001.0 SP1' when 14 then '12.0.5000.0 SP2'  WHEN 12 THEN '11.0.6020.0, SP3' WHEN 82 THEN '10.50.6000.34, SP3' WHEN 8 THEN '10.0.6000.29, SP4' WHEN 5 THEN '9.0.5000, SP4' END,'','','','','','','','','','','','','','', '','','','','','','','','','','','';
...
-----  Configuration status for summary

DECLARE @HAConfigStatus varchar(255)
DECLARE @HAConfig_State int


SET @HAConfigStatus = ( select count(1) from ##HACONFIG where  Mirroring_Status <> 'N/A' and LS_Status <> 'N/A' and Repl_Status <> 'N/A' and AG_Status <> 'N/A')

IF @HAConfigStatus > 0
	BEGIN
	SET @HAConfigStatus = ( select count(1) from ##HACONFIG where  Mirroring_Status <> 'GOOD' and LS_Status <> 'GOOD' and Repl_Status <> 'GOOD' and AG_Status <> 'GOOD' )
		IF @HAConfig_State > 0
...
DROP TABLE dbo.##DBCONFIGURATION
...
BEGIN
	...
	AS
	(
		SELECT drcs.database_name, ar.secondary_role_allow_connections_desc
	...
	WHERE sysDB.name not in (select database_name from AOAGDBs where secondary_role_allow_connections_desc = 'NO')
END

ELSE -- if SQL version less than 2012
BEGIN
	INSERT INTO ##DBCONFIGURATION
	SELECT
	sysDB.database_id,
...
DECLARE dbstatuswarning_cursor CURSOR LOCAL FAST_FORWARD FOR
...
INSERT INTO ##ReviewResults SELECT 'DB Configuration Status','Database Id','Database Name','Date Created','DB Owner','DB Status','Recovery Mode','Collation'	,'User Access',	'Compatibility_Level','Page Verify Option',	'ReadOnly or Read_Write','Auto Close Status','Auto Shrink Status',	'Auto Create Stats Status',	'Auto Update Stats Status',	'Full Text Status',
	'Is_Database_Trustworthy',	'Is_Database_Encrypted',	'Is_CDC_Enabled',	'Is_Database_Published','Is_Database_Subscribed','LSConfigured','Mirror State Desc','Partner Name',	'Mirror Role','Safety Level','Witness','';
...
DECLARE db_cursor CURSOR LOCAL FAST_FORWARD FOR
	SELECT name FROM ##DBCONFIGURATION
...
WHILE @@fetch_status = 0
BEGIN
	SET @CursorSQL = 'INSERT INTO #DBInfo Execute (''DBCC DBINFO ('+ @curdb +') WITH TABLERESULTS''); 
...
INSERT INTO dbo.##ReviewResults
SELECT DatabaseName, LastDBCCCheckDB_RunDate,DBCC_Status,'','','','','','','','','','','','','','', '','','','','','','','','','','','' FROM #Value
...
INSERT INTO dbo.##ReviewResults SELECT '', '','','','','','','','','','','','','','','','', '','','','','','','','','','','','';
 ...
 SET @DBCCStatus = (select count(DBCC_Status) from #Value where DatabaseName <> 'tempdb' and DBCC_Status IN ( 'WARNING', 'Never Run'))
...
DECLARE drivesummary_cursor CURSOR LOCAL FAST_FORWARD FOR
SELECT [% Free Space]
FROM dbo.##DrvInfo
...
INSERT INTO dbo.##ReviewResults SELECT '', '','','','','','','','','','','','','','','','', '','','','','','','','','','','','';
INSERT INTO dbo.##ReviewResults SELECT 'DATABASE SPACE UTILIZATION CHECK', '','','','','','','','','','','','','','','','', '','','','','','','','','','','','';
INSERT INTO dbo.##ReviewResults SELECT '', '','','','','','','','','','','','','','','','', '','','','','','','','','','','','';
...
BEGIN
	...
	insert into dbo.##Tbl_ValidDbs
	select name from sys.databases
	except
	select database_name from AOAGDBs
...
IF UPPER(ISNULL(@Level, 'DATABASE')) = 'DATABASE'
  BEGIN
    ...
    IF @TargetDatabase IS NULL
	  ...
	  INSERT INTO dbo.##ReviewResults SELECT '', '','','','','','','','','','','','','','','','', '','','','','','','','','','','','';
	  INSERT INTO dbo.##ReviewResults SELECT 'Unit','Used Space', 'Free Space','Total Space','Ddata File Space','Log File Space','','','','','','','','','','','','', '','','','','','','','','','','';
	  INSERT INTO dbo.##ReviewResults  
      SELECT CASE WHEN @Unit = 'GB' THEN 'GB' WHEN @Unit = 'KB' THEN 'KB' ELSE 'MB' END AS 'SUM', 
      SUM (used) AS 'Used', 
      SUM (free) AS 'Free', 
...
DECLARE backupswarning_cursor CURSOR LOCAL FAST_FORWARD FOR
...
DECLARE backupsummary_cursor CURSOR LOCAL FAST_FORWARD FOR
SELECT warning
...
DECLARE jobsummary_cursor CURSOR LOCAL FAST_FORWARD FOR
